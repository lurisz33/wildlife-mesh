...
extern LoraMesher& mesh;

char DTfilenamebuff[DTfilenamesize];
uint8_t DTDestinationNode = 0x9F6C;
...
void setDTDestination(uint8_t nodeID) {
    DTDestinationNode = nodeID;
}

// Send a DT packet over the mesh instead of raw radio:
uint16_t meshTransmitDT(
    uint8_t* hdr, 
    uint8_t  hdrL,
    uint8_t* data, 
    uint8_t  dataL
  ...
  // pack header + data
  memcpy(buf,           hdr,  hdrL);
  memcpy(buf + hdrL,    data, dataL);
  // send via LoRaMesher
  mesh.createPacketAndSend(DTDestinationNode, buf, hdrL + dataL);
  return hdrL + dataL;
}

// Wait for an ACK packet to arrive via mesh and decode it into `DTheader[]`
uint8_t meshWaitACKDT(uint8_t* hdr, uint8_t hdrL, uint16_t timeout)
{
  ...
  while (millis() - start < timeout)
  {
    ...
    AppPacket<uint8_t>* packet = mesh.getNextAppPacket<uint8_t>();
    if (packet != nullptr)
    {
      uint8_t from = packet->src;
      memcpy(hdr, packet->payload, packet->payloadSize);
      uint8_t packetType = hdr[0];
      
      delete packet;
      
      // List of valid ACK types
      if (packetType == DTSegmentWriteACK || 
          packetType == DTFileOpenACK || 
          packetType == DTFileCloseACK ||
          packetType == DTSegmentWriteNACK ||
          packetType == DTStartNACK) {
          return 1;  // Valid ACK/NACK
      }
    }
  }
  return 0;  // timeout
...
// Replace raw receiveDT: pull from mesh instead
// Replace raw receiveDT: pull from mesh instead
int meshReceiveDT(
  uint8_t* hdr, 
  uint8_t  hdrMax, 
  ...
  while (millis() - start < timeout) {
    yield();
    auto* packet = mesh.getNextAppPacket<uint8_t>();
    if (!packet) continue;

    int totalLen = packet->payloadSize;
    if (totalLen < 3) { delete packet; continue; }

    memcpy(hdr, packet->payload, totalLen);
    int hL = hdr[2];
    if (hL > hdrMax || totalLen < hL) { delete packet; continue; }

    int dL = totalLen - hL;
    if (dL > dataMax) { delete packet; continue; }

    if (dL) memcpy(data, packet->payload + hL, dL);
    delete packet;
    return totalLen;
  }
  return 0;
...
// Send an ACK packet over mesh
void meshSendACKDT(uint8_t* hdr, uint8_t hdrL, int8_t power)
{
  uint8_t destination = DTheader[1];
  
  if (destination == 0 || destination == 255) {
    Serial.println(F("Warning: Invalid ACK destination"));
    return;
  }
  
  mesh.createPacketAndSend(destination, hdr, hdrL);
}

bool sendFile(char *DTFileName, uint8_t namelength, uint8_t destination = GATEWAY_ID) {
  DTDestinationNode = destination;
  //**************************************************************************************************************
...
bool sendFileSegment(uint16_t segnum, uint8_t segmentsize)
{
  ...
  do
  {
    ...
    TXPacketL = meshTransmitDT(
  DTheader, DTSegmentWriteHeaderL,
  DTdata,  segmentsize
);
    ...
    ValidACK = meshWaitACKDT
    (DTheader, DTSegmentWriteHeaderL, ACKsegtimeoutmS);
...
bool startFileTransfer(char *buff, uint8_t filenamesize, uint8_t attempts)
{
  ...
  DTdata,  segmentsize
    ...
    ValidACK = meshWaitACKDT(DTheader, DTFileOpenHeaderL, ACKopentimeoutmS);
    RXPacketType = DTheader[0];
...
bool endFileTransfer(char *buff, uint8_t filenamesize)
{
  ...
  DTdata,  segmentsize
    ...
    ValidACK = meshWaitACKDT(DTheader, DTFileCloseHeaderL, ACKclosetimeoutmS);
    RXPacketType = DTheader[0];
...
void build_DTFileOpenHeader(uint8_t *header, uint8_t headersize, uint8_t datalength, uint32_t filelength, uint16_t filecrc, uint8_t segsize)
{
  ...
  arrayWriteUint8(mesh.getLocalAddress());                 //byte 1, initial DTflags byte, not used here
...
void build_DTSegmentHeader(uint8_t *header, uint8_t headersize, uint8_t datalen, uint16_t segnum)
{
  ...
  arrayWriteUint8(mesh.getLocalAddress());                    //initial DTflags byte, not used here
...
bool processFileClose()
{
  ...
  digitalWrite(DTLED, HIGH);
  meshSendACKDT(DTheader, DTFileCloseHeaderL, TXpower);
  digitalWrite(DTLED, LOW);
...
bool processFileOpen(uint8_t *buff, uint8_t filenamesize)
{
  ...
  digitalWrite(DTLED, HIGH);
  meshSendACKDT(DTheader, DTFileOpenHeaderL, TXpower);
  digitalWrite(DTLED, LOW);
...
bool processSegmentWrite()
{
  ...
  if (!DTFileOpened)
  {
    ...
    delay(ACKdelaymS);
    digitalWrite(DTLED, HIGH);
    meshSendACKDT(DTheader, DTStartHeaderL, TXpower);
    digitalWrite(DTLED, LOW);
    return false;
  }
  ...
  if (DTSegment == DTSegmentNext)
  {
    ...
    delay(ACKdelaymS);
    digitalWrite(DTLED, HIGH);
    meshSendACKDT(DTheader, DTSegmentWriteHeaderL, TXpower);
    digitalWrite(DTLED, LOW);
    DTReceivedSegments++;
...
bool receiveaPacketDT()
{
  ...
  RXPacketL = meshReceiveDT(DTheader, HeaderSizeMax, (uint8_t *) DTdata, DataSizeMax, NetworkID, RXtimeoutmS, WAIT_RX);
  digitalWrite(DTLED, HIGH);
  ...
  {
    //if the meshReceiveDT() function detects an error RXOK is 0
